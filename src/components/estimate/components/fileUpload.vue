...
const handleDeleteExistingFile = (imageId: number) => {
    confirm.require({
        ...
        accept: async () => {
            ...
            try {
                await api.delete(`/estimate-images/${imageId}`);
...
const onSelectedFiles = (event: any) => {
    newFiles.value = event.files as File[];
    syncFiles(newFiles.value);
};
...
.img-thumb:hover :deep(.thumb-delete) {
    opacity: 1 !important;
...
:deep(.thumb-delete .p-button-icon) { font-size: 0.5rem !important; }
...
.galleria-thumb-img { width: 60px; height: 60px; object-fit: cover; display: block; border-radius: 6px; }

.galleria-footer {
    display: flex; align-items: center; justify-content: space-between;
    gap: 0.75rem; padding: 0.55rem 1rem;
    background: rgba(10,15,26,0.95); border-top: 1px solid #1e293b;
    font-family: 'Tajawal', 'Cairo', sans-serif; flex-wrap: wrap;
}
.galleria-footer-info { display: flex; align-items: center; gap: 0.5rem; min-width: 0; }
.lb-controls { display: flex; align-items: center; gap: 0.3rem; direction: ltr; }
.lb-counter  { font-size: 0.68rem; font-weight: 700; color: #475569; background: #1e293b; padding: 0.15rem 0.5rem; border-radius: 99px; white-space: nowrap; }
.lb-name     { font-size: 0.75rem; color: #94a3b8; font-weight: 500; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 200px; }
.ctrl-btn {
    width: 28px; height: 28px; border-radius: 7px;
    background: #1e293b; border: 1.5px solid #334155; color: #64748b;
    display: flex; align-items: center; justify-content: center;
    font-size: 0.65rem; cursor: pointer; transition: all 0.15s; text-decoration: none;
}
